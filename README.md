...
| `--no-summary` | off | Do not build `licenses_summary.json` after processing (summary is on by default). |
...
# Multiple repos from the command line (summary is written by default)
python sbom_licenses.py --repos owner/repo1,owner/repo2

# Skip writing the summary
python sbom_licenses.py -f repos.json --no-summary
...
The summary is **on by default**: whenever you process repo(s), the script writes `licenses_summary.json` in the output directory. Use `--no-summary` to skip it. Use `--summary-only` to build only the summary from existing result files (no cloning). It **does not remove** any repo-specific `licenses_<repo_slug>.json` files. The summary hierarchy is:
